...
The public registry for the Daml DARs [C7.digital](https://c7.digital/) builds
for the Canton Network — the app models behind 7Trust and 7LOCK, plus the
app-agnostic libraries any Canton app can build on.

Every DAR ships as a GitHub Release asset. There are no `.dar` files in this
git tree.

## Which DAR do you want?

Pick a row and click its version to download. Those links are **versioned and
immutable** — regenerating this table points them at a newer release, but no
published URL ever changes meaning.
...
The links above are the quickest way in. For a build, pin the tag instead so
the version is recorded where someone can read it:
...
package feed pointing at this registry, so a DAR reaches a node when an operator
puts it there.
...
Releases are namespaced **per stream** so multiple apps — and the shared
libraries they build on — coexist without tag collisions. Tags are
`<stream>/v<version>`; every asset is `<daml-package>-<version>.dar`.

Every release is immutable, and **every asset carries its version in its
filename**, so a DAR stays identifiable once it has been downloaded. Releases
are published with `make_latest: false`, so this repository's `/releases/latest`
is not a meaningful pointer — always pin a `<stream>/v<ver>` tag.
